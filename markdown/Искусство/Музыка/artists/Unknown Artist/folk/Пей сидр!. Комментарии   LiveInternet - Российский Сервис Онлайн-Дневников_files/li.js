						...
						new Ajax.Request('/journal_addcomment.php?action=newcomment&doajax=1', {
							...
							onComplete:function(transport){
								...
								/* -[ Show success notify ]- */
									...
									LiCi.Notify.show('true','Комментарий был добавлен...');
				...
				LI.fsearch.insert = function (obj,e,str) {
					...
					if (sorted.length > 0) {					
						...
						for (var j = 0; j < sorted.length; j++) {
							...
							list += '<p>'+
									'<a href="'+itm.domain+'profile" class="bl-fsrch_list_grp_itm_ava" style="background:#FFF url(http://ma.li.ru/'+itm.domain+'/.gif) no-repeat 50% 50%;" href="#">Аватар '+itm.name+'</a>'+
		...
		/* -----------[ Event ]----------- */
			...
			/* -[ Like ]- */
				LI.likethis.like = function( e ) {
					LI.likethis.event(e, 'Отменить', 'plus', function(){
						LI.likethis.unlike( e );
					});
				}
			/* -[ Unlike ]- */
				LI.likethis.unlike = function( e ) {
					LI.likethis.event(e, 'Понравилось', 'minus',  function(){
						LI.likethis.like( e );
					});
				}			
