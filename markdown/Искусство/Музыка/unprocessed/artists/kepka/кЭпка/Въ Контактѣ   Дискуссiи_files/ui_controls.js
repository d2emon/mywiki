...
createChildClass('Selector', UiControl, {
  ...
  initDropdown: function() {
    this.scrollbarWidth = browser.safari_mobile ? 20 : window.sbWidth();
  ...
  handleKeyboardEvent: function(e) {
    ...
    case 'keypress':
      if (e.which == KEY.RETURN && browser.opera && self.options.enableCustom && (self.select.selectedItem() === null)) {
        self.select.hide();
        self.input.blur();
        return false;
    ...
    case 'keydown':
      switch(e.keyCode) {
        ...
        break;
        case KEY.RETURN:
          if (!browser.opera && self.options.enableCustom && (self.select.selectedItem() === null)) {
            self.select.hide();
            self.input.blur();
            return false;
          }
  ...
  handleKeyboardEventOutside: function(e) {
    ...
    switch(e.keyCode) {
      ...
      case KEY.DEL:
        if (this.selectedTokenId) {
          var nextTokenId = 0;
          for (var i = this._selectedItems.length - 2; i >= 0; i--) {
            if (this._selectedItems[i][0] == this.selectedTokenId && this._selectedItems[i+1]) {
              nextTokenId = this._selectedItems[i+1][0];
            }
          }
          this.removeTagData(this.selectedTokenId);

          if (nextTokenId) {
            this.selectToken(nextTokenId);
          } else if (!this.readOnly && !this.hasFocus) {
            this.input.focus();
          }
        } else if (this.hasFocus && this._selectedItems.length) {
          this.selectToken(this._selectedItems[this._selectedItems.length - 1][0]);
        }
        cancelEvent(e);
        return false;
        break;
  ...
  onInputClick: function(e) {
    ...
    if (!self.readOnly) self.input.focus();
  ...
  _selectItem: function(item, fireEvent, setFocus) {
    if (item == null) {
      return;
    }
    ...
    if (data[0] === this.resultField.value) {
      if (!this.options.multiselect) {
        ...
        if (this.input.value.length || !this.options.placeholder) {
          ...
          if (!this.disabled) this.input.style.color = '#000';
        } else {
          this.input.value = this.options.placeholder;
          if (!this.disabled) this.input.style.color = this.options.placeholderColor;
        }
    ...
    this.updateInput();
    if ((setFocus || this.options.multiselect) && !this.readOnly) {
      setTimeout(function() {
        this.focusSelf = true;
        hide(this.input);
        show(this.input);
  ...
  showDefaultList: function() {
    if (this.defaultList && this.select.hasItems()) {
      ...
      this._showSelectList(text, this.options.defaultItems.length ? this.options.defaultItems : this.dataItems);
    }
  },
  ...
  focus: function() {
    if (!this.readOnly) this.input.focus();
  },
...
createChildClass('Select', UiControl, {
    ...
    switch(e.keyCode) {
      ...
      case KEY.TAB:
        if (isFunction(this.options.onItemSelect) && this.active > -1) {
          this.options.onItemSelect(this.selectedItem());
          return false;
        }
        this.hide();
      break;
  ...
  removeItem: function(value) {
    ...
    for (var i = 0, len = this.list.childNodes.length; i < len; ++i) {
      ...
      if (!i && node.nextSibling) {
        addClass(node.nextSibling, this.CSS.FIRST);
      } else if (i == len - 1) {
        addClass(this.list.childNodes[i - 1], this.CSS.LAST);
      }
      break;
    }
    this.updateContainer();
...
function Radiobuttons(input, buttons, options) {
  ...
  each(buttons, function(i, v) {
    new Radiobutton(ge(id + v[0]), {label: v[1], width: options.width});
  });
...
createChildClass('DropdownMenu', UiControl, {
  ...
  initDOM: function(input, options) {
    ...
    this.header = ce('div', {
      ...
      innerHTML: '<div>' + this.options.title + '</div>'
    });
  ...
  afterInit: function(items, options) {
    ...
    onDomReady(function() {
      ...
      var header = self.header, body = self.body;
      ge('dd_rows_'+ self.guid).appendChild(self.rows);
      self.setOptions(self.options);
      if (self.options.target) {
        self.options.target.onclick = function() {self.show();return false;};
      }
      if (self.options.target && self.options.showHover) {
        ...
        addEvent(self.options.target, 'mouseover', function() {
  ...
  addItem: function(item) {
    ...
    var link = ce('a');
    if(isArray(item))item = {i: item[0], l: item[1], onClick: item[2], c: item[3], s: item[4], b: item[5], h: item[6]};
    ...
    addEvent(link, 'click', function(e) {
      ...
      if(self.options.updateTarget && hide) {
        ...
        if(self.options.target)self.options.target.innerHTML = text;
      }
    });
  ...
  handleMouseEvent: function(e) {
    clearTimeout(e.data.self.mouseTimer);
    if (e.type == 'mouseout') {
      e.data.self.mouseTimer = setTimeout(e.data.self.hide.bind(e.data.self), 400);
    }
  }
});
...
createChildClass('Indexer', UiUtil, {
  ...
  setData: function(data) {
    ...
    setTimeout(function() {this.createIndex();}.bind(this), 10);
  },
  createIndex: function() {
    if (!this.storage.data.length) return;
    debugLog('createIndex start, ' + this.storage.data.length + ' items');
    each(this.storage.data, this.indexItem.bind(this));
    debugLog('createIndex ended');
  },
  indexItem: function(k, v) {
    var i, j, current_words = '', index_key;
    for (i = 0; i < this.options.indexkeys.length; i++) {
      if (!v[this.options.indexkeys[i]]) continue;
      current_words += ' ' + v[this.options.indexkeys[i]].replace(this.options.delimeter, ' ').replace('<b>', '').replace('</b>', '');
    }
    current_words = trim(current_words.toLowerCase()).split(' ');
  ...
  search: function(pattern) {
    debugLog('search start, index width: ' + this.options.chars + ', data size: ' + this.storage.data.length);
    pattern = trim(pattern.toLowerCase().replace(this.options.delimeter, ' '));
    debugLog('pattern length: ' + pattern.length);
    if (!pattern) {
      debugLog('empty pattern, return whole list');
      return self.storage.data;
    }
    var self = this;
    if (pattern.length <= this.options.chars && pattern.indexOf(' ') == -1) {
      debugLog('found whole pattern indexed');
      var retArr = [];
    ...
    var ret_arr = [];
    debugLog('index returned: ' + min_size + ' items');
    if (!min_size) return ret_arr;
    debugLog('starting manual filter');
    ...
    debugLog('manual filter ended, found ' + ret_arr.length + ' items');
    return ret_arr;
  },
  flush: function() {
    delete this.storage;
