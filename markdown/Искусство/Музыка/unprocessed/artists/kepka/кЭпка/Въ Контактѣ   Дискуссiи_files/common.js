...
// hideQDrop(qCur);
 ...
 if (l) {
  for (var i = 0; i < l; i++) {
   ...
   var lnk = '<a href="'+base_domain+'mail.php?act=write&to='+res[i][0]+'" onclick="cancelEvent(event); return showBox(\'sendMsg\', \'mail.php\', {act: \'a_write_box\', to: '+res[i][0]+'}, true, true, {title: \''+(window['profile_newmsg_box_title'] || 'Новое сообщение')+'\', progress: \'sending_progress\', width: 450}, [\'css/mail2.css\', \'js/mail.js\']);" class="qfr_lnk">'+(window['global_write_msg'] || 'Написать сообщение')+'</a>';
...
function genQDrop() {
  ...
  for (var i = 0; i < qArr.length; i++) {
    ...
    if (i == qArr.length - 1) {qoff += ' qdiv_bottom'; qover += ' qdiv_bottom';}
...
function getQCur(n) {
  var cur = qArr[0];
  each(qArr, function(i,v){if(v[0] == n) cur=v; });
  return cur;
}
...
function qDoSearch() {
 ...
 if (!qCur && qi.value) {names_only = "&name=1";}
 window.location = base_domain+'gsearch.php?section='+qArr[qCur][3]+'&q='+qi.value+names_only;
}
...
function MessageBox(options) {
  ...
  function addButton(options) {
    ...
    var buttonWrap = ce('div', {className: options.style, innerHTML: '<div id="button' + guid + '_' + buttonsCount + '">' + options.label + '</div>'});
    if (boxProgress) {
...
function TabbedBox(curTab, tabs, options) {
  ...
  var mb = options.onDone ? new AlertBox(options.title||'', '', options.onDone, extend(options, {boxType:'CONFIRM'})) : new MessageBox(options), body = mb.body(), cur = curTab, contents = {}, contentBox, w, html = '<div class="mb_header">'+(options.header||'')+'</div><div class="mb_tabs">', tmp = {}, curContent = '';
  options = extend({onTab:function(tab, loaded){}, createContent:function(content){return content || '';}, tabLoaded:function(tab){}}, options);
  ...
  function sh(o,m,c) { return '<div class="mb_shadow'+(c||'')+'" style="opacity:'+(o/100)+';filter:alpha(opacity='+o+');margin-top:'+m+'px;width:'+w+'px"></div>'; }
  function err(text) { contentBox.innerHTML = text; }

  w = getSize(body.parentNode.parentNode)[0] - 22;
  html += '<div class="fl_r mb_search">'+(options.search||'')+'</div><div class="clear"></div></div>'+sh(30,0)+sh(11,1)+sh(7,2)+sh(3,3)+'<div class="mb_tab_content" style="'+(options.tabStyle||'')+'"></div>'+sh(3,-4)+sh(7,-3)+sh(11,-2)+sh(30,-1)+sh(80,0,' mb_shadow_last');
  mb.content(html);
  ...
  switchTab: function(tab) {
    ...
    curContent = contents[tab];
    contentBox.innerHTML = options.createContent(curContent);
    return true;
  },
      ...
      onSuccess: function(o, t) {
        if (ev) {
          try {
            ...
            if (res.script)  window.execScript ? window.execScript(res.script) : eval.call(window, res.script);
...
function BaseTooltip(target, options) {
  var defaults = {
    ...
    toUp: true,
    template:
  '<table class="tooltip_wrap" cellpadding="0" cellspacing="0">\
    <tr>\
      <td colspan="3">\
        <div class="tt_top_pointer"></div>\
      </td>\
    </tr>\
    <tr>\
      <td class="tt_left_shadow"></td>\
      <td>{content}</td>\
      <td class="tt_right_shadow"></td>\
    </tr>\
    <tr>\
      <td colspan="3">\
        <div class="tt_bottom_pointer"></div>\
        <div class="tt_bottom_shadow"></div>\
      </td>\
    </tr>\
  </table>'
  };
  ...
  each(this.options.params, function(i,v){
    t.content = t.content.replace('{'+i+'}', v||'');
  });

  var container = ce('div', {
      className: this.options.className,
      innerHTML: this.options.template.replace('{content}', this.content)
    }, {
  ...
  this.height = false;
  this.bottomPointer = geByClass('tt_bottom_pointer', container)[0];
  document.body.appendChild(container);
  if (!isFunction(this.options.onInit) || this.options.onInit.call(this) !== false) {
  ...
  this.show = function () {
    var t = this;
    if (!this.height) setTimeout(function(){t.show();}, 300);
    var tcoords = getXY(target);
    var x = tcoords[0] - 40, y, starty;
    var tsize = getSize(target);
    var scrollTop = Math.max(document.getElementsByTagName('html')[0].scrollTop, document.getElementsByTagName('body')[0].scrollTop);
    var scrollHeight = Math.max(document.getElementsByTagName('html')[0].scrollHeight, document.getElementsByTagName('body')[0].scrollHeight);
    var noUp = (tcoords[1] - scrollTop < this.height + 10), noDown = (scrollHeight - scrollTop - tcoords[1] - tsize[1] < this.height + 10);
    var toUp = this.options.toUp ? (!noUp || noDown) : (noDown && !noUp);
    if (!toUp) {
      y = tcoords[1] + tsize[1] + 1;
      starty = y + 15;
      addClass(container, 'base_tooltip_top');
      removeClass(container, 'base_tooltip_bottom');
      if (browser.msie && !browser.msie8) setStyle(this.bottomPointer, 'visibility', 'hidden');
    } else {
      y = Math.max(tcoords[1] - this.height - 3, 0);
      addClass(container, 'base_tooltip_bottom');
      removeClass(container, 'base_tooltip_top');
      starty = y - 15;
      if (browser.msie) {
        if (!browser.msie8) setStyle(this.bottomPointer, 'visibility', 'visible');
        else animate(this.bottomPointer, {opacity: 1}, 200);
      }
    }
    setStyle(container, {display: 'block', top: starty, left: x, opacity: 0});
    animate(container, {opacity: 1, top: y}, 200);
  }
  if (isFunction(this.options.onMouseOver)) addEvent(container, 'mouseover', this.options.onMouseOver);
  if (isFunction(this.options.onMouseOut)) addEvent(container, 'mouseout', this.options.onMouseOut);

  this.hide = function () {
    if (browser.msie8 && isVisible(this.bottomPointer)) fadeOut(this.bottomPointer, 100);
...
function showTT(anchor, func, url, params) {
  ...
  if (!tooltip) {
    var hideTO = function () {
      if (!data(anchor, 'over') && data(anchor, 'tooltip')) data(anchor, 'tooltip').hide();
    };
    clearTimeout(data(anchor, 'preview_timeout'));
    var t = setTimeout(function () {
      if (!data(anchor, 'over')) return;
      var onDone = function (o, t) {
        if (!data(anchor, 'over')) return;
        var res = o ? eval('('+t+')') : t;
        if (res.js) res.onInit = function(){try{eval(res.js); return ttToShow;}catch(e){}};
        tooltip = new func(anchor, extend(res, {
          onMouseOut: function() {data(anchor, 'over', false); setTimeout(hideTO, 400);},
          onMouseOver: function () {data(anchor, 'over', true);}
        }));
        data(anchor, 'tooltip', tooltip);
        tooltip.show();
      };
      if (url) {
        Ajax.Post({url:url, query:params, onDone:onDone});
      } else {
        onDone(null, params);
      }
    }, 200);
    data(anchor, 'preview_timeout', t);
    addEvent(anchor, 'mouseout', function () {data(anchor, 'over', false); setTimeout(hideTO, 400);});
  } else {
    setTimeout(function () {
      if (!data(anchor, 'over')) return;
